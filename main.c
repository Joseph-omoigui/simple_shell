 ...
 * @env: the num of values received from the command line
 * Return: 0 on succes.
 */
...
/**
 * handle_ctrl_c - always print the prompt in a new line
...
/**
 * initialize_data - initialize the struct with the information of the program
...
void sisifo(char *prompt, data_of_program *data)
{
	...
	while (++(data->exec_counter))
	{
		...
		{
			free_all_data(data);
			exit(errno); /* if the EOF is the fisrt Char of string, exit*/
		}
		if (string_len >= 1)
		{
			expand_alias(data);
			...
			tokenize(data);
			if (data->tokens[0])
			{ /* if a text is given to prompt, then execute */
